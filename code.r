...
data <- read.csv("data_oecd.csv",sep="\t")
data <- subset(data, year < 2010)
eu14 <- subset(data,cty=="Austria"|cty=="Belgium"|cty=="Denmark"|cty=="Finland"|cty=="France"|cty=="Germany"|cty=="Greece"|cty=="Ireland"|cty=="Italy"|cty=="Luxembourg"|cty=="Netherlands"|cty=="Portugal"|cty=="Spain"|cty=="Sweden")
eu15 <- subset(data,cty=="Austria"|cty=="Belgium"|cty=="Denmark"|cty=="Finland"|cty=="France"|cty=="Germany"|cty=="Greece"|cty=="Ireland"|cty=="Italy"|cty=="Luxembourg"|cty=="Netherlands"|cty=="Portugal"|cty=="Spain"|cty=="Sweden"|cty=="United Kingdom")
uk <- subset(data, cty=="United Kingdom")

# Collapsed data
eu14.summary <- ddply(eu14, .(year), summarise, sum.gdp = sum(hexp_gdp,na.rm=TRUE),mean.gdp=mean(hexp_gdp,na.rm=TRUE), sum.ppp = sum(hexp_ppp,na.rm=TRUE), mean.ppp=mean(hexp_ppp,na.rm=TRUE))
eu15.summary <- ddply(eu15, .(year), summarise, sum.gdp = sum(hexp_gdp,na.rm=TRUE),mean.gdp=mean(hexp_gdp,na.rm=TRUE), sum.ppp = sum(hexp_ppp,na.rm=TRUE), mean.ppp=mean(hexp_ppp,na.rm=TRUE))
yrange <- range(uk$year)

# Definition of UK government party
gov <- read.csv("data_uk.csv")
...
fig1.hexp_gdp <- ggplot() + 
	geom_boxplot(data = eu15, aes(x=year,y=hexp_gdp,group=year),colour="darkgrey",outlier.colour="darkgrey") +
	...
	scale_area(name="PPP") + scale_colour_manual("GDP", c("EU" = cols[2], "UK" = cols[1])) +
...
fig2.hexp_ppp <- ggplot() + 
	...
	scale_area(name="GDP") + scale_colour_manual("PPP", c("EU" = cols[2], "UK" = cols[1])) +
...
fig3.contrib_density <- ggplot(eu15) + 
	geom_density(aes(x=contrib[cty!="United Kingdom"],colour="EU"),fill=cols[2],alpha=0.2,binwidth=.5,na.rm=TRUE) + 
	geom_density(aes(x=contrib[cty=="United Kingdom"],colour="UK"),fill=cols[1],alpha=0.2,binwidth=.5,na.rm=TRUE) +
	scale_colour_manual("", c("EU" = cols[2], "UK" = cols[1])) +
...
fig4.comp_gdp <- ggplot() +
	geom_line(data=eu14.summary,aes(x=year,y=mean.gdp,colour="EU")) +
	geom_line(data=uk,aes(x=year,y=hexp_gdp,colour="UK")) +
	scale_colour_manual("", c("EU" = "black", "UK" = "darkgrey")) +
	geom_rect(data=gov,aes(NULL, NULL, xmin = start, xmax = end, fill = party), ymin = gdp.min, ymax = gdp.max) +
...
fig5.comp_ppp <- ggplot() +
	...
	geom_line(data=uk,aes(x=year,y=hexp_ppp,colour="UK")) +
	scale_colour_manual("", c("EU" = "black", "UK" = "darkgrey")) +
	geom_rect(data=gov,aes(NULL, NULL, xmin = start, xmax = end, fill = party), ymin = ppp.min, ymax = ppp.max) +
	scale_fill_manual("", values = alpha(c("blue", "red"), 0.2)) +
...
fig6.ratios_gdp <- ggplot(uk) +
	...
	scale_colour_manual("", c("EU-14" = "black","EU-15" = "darkgrey")) +
	geom_rect(data=gov,aes(NULL, NULL, xmin = start, xmax = end, fill = party), ymin = rmin.gdp, ymax = rmax.gdp) +
	scale_fill_manual("", values = alpha(c("blue", "red"), 0.2)) +
...
fig7.ratios_ppp <- ggplot(uk) +
	...
	scale_colour_manual("", c("EU-14" = "black","EU-15" = "darkgrey")) +
	geom_rect(data=gov,aes(NULL, NULL, xmin = start, xmax = end, fill = party), ymin = rmin.ppp, ymax = rmax.ppp) +
	scale_fill_manual("", values = alpha(c("blue", "red"), 0.2)) +
...
fig8.ratios <- ggplot(uk) +
	...
	scale_colour_manual("", c("GDP" = "black", "PPP" = "darkgrey")) +
	geom_rect(data=gov,aes(NULL, NULL, xmin = start, xmax = end, fill = party), ymin = rmin, ymax = rmax) +
	scale_fill_manual("", values = alpha(c("blue", "red"), 0.2)) +
...
fig9.ratios_a <- ggplot(uk) +
	...
	scale_colour_manual("", c("GDP" = "black", "PPP" = "darkgrey")) +
	geom_rect(data=gov,aes(NULL, NULL, xmin = start, xmax = end, fill = party), ymin = rmin.a, ymax = rmax.a) +
	scale_fill_manual("", values = alpha(c("blue", "red"), 0.2)) +
...
fig10.fitted.smoothers <- ggplot() + 
	 ...
	 scale_colour_manual("", c("GAMM" = cols[1], "AR(2)" = cols[2])) +
...
fig12.fitted.changes <- ggplot() +
	...
	scale_colour_manual("", c("Actual" = "black", "Fitted" = "darkgrey","Increase"="red")) +
	geom_rect(data=gov,aes(NULL, NULL, xmin = start, xmax = end, fill = party), ymin = min(uk$hexp_gdp), ymax = max(uk$hexp_gdp)) +
	scale_fill_manual("", values = alpha(c("blue", "red"), 0.2)) +
